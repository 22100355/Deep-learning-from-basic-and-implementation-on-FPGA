...
architecture behave of lenet5_control_s_axi is
    ...
    constant ADDR_CLASSES_DATA_0 : INTEGER := 16#10#;
    constant ADDR_CLASSES_CTRL   : INTEGER := 16#14#;
    constant ADDR_IMAGE_R_DATA_0 : INTEGER := 16#18#;
    constant ADDR_IMAGE_R_CTRL   : INTEGER := 16#1c#;
    ...
    signal int_classes         : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CLASSES_DATA_0 =>
                        rdata_data <= RESIZE(int_classes(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    classes              <= STD_LOGIC_VECTOR(int_classes);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CLASSES_DATA_0) then
                    int_classes(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_classes(31 downto 0));
                end if;
            end if;
        end if;
    end process;
