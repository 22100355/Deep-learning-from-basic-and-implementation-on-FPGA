...
#-------------------------------------------------------------------------------
__author__     = "Ando Ki"
__copyright__  = "Copyright 2021-2024 Ando Ki"
...
def plot_histogram( ww
    ...
    if True: plt.pause(1)
        ...
        while mouseClick == False:
            mouseClick = not plt.waitforbuttonpress(-1)
                             # This will return True is a key was pressed,
                             # False if a mouse button was pressed and
                             # None if timeout was reached without either being pressed.
...
if __name__ == '__main__':
    ...
    if _torchsummary:
        summary_str, (total_params, trainable_params) = torchsummary.summary(model, (args.input_channels, 32, 32))
        if args.verbose:
            print(summary_str)
    else:
        model_stats = torchinfo.summary(model, (1, args.input_channels, 32, 32), verbose=0)
...
# Revision history:
#
# 2024.06.03: 'torchinfo' added instead of 'torchsummary'.
